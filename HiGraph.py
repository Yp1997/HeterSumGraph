...
class HSumGraph(nn.Module):
    ...
    def forward(self, graph, ret_sent_state=False):
        """
...
class HSumGraphCent(nn.Module):
    """ without sent2sent and add residual connection """
    def __init__(self, hps, embed, lambda1=0.6):
        """
        ...
        self.n_feature = hps.hidden_size
        self.wh = nn.Linear(self.n_feature+1, 2)
    ...
    def forward(self, graph):
        ...
        num_sents, _ = sent_state.size()
        forward_matrix = torch.zeros(num_sents, 1).to(torch.device("cuda"))
        backward_matrix = torch.zeros(num_sents, 1).to(torch.device("cuda"))
        cos = self.cos
        ...
        for j in range(len(glist)):
            ...
            for s in range(num_sents):
                f_len = 0
                for f in range(0, s):
                    f_sim = cos(cur_sent_state[s], cur_sent_state[f])
                    forward_matrix[st_idx + s] += f_sim
                    f_len += 1
                # normalize
                if (f_len != 0):
                    forward_matrix[st_idx: st_idx + s] /= f_len

                b_len = 0
                for b in range(s+1, num_sents):
                    b_sim = cos(cur_sent_state[s], cur_sent_state[b])
                    backward_matrix[st_idx + s] += b_sim
                    b_len += 1

                # normalize
                if (b_len != 0):
                    backward_matrix[st_idx: st_idx + s] /= b_len
        
        central_scoring = self.lambda1 * forward_matrix + (1 - self.lambda1) * backward_matrix
        sent_state = torch.cat((sent_state, central_scoring), 1)
...
class HSumGraphDpeak(nn.Module):
    """ without sent2sent and add residual connection """
    def __init__(self, hps, embed, lambda1=0.6):
        """
